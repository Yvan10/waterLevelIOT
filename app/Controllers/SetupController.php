...
class SetupController extends Controller
{
    ...
    {
        $form = $this->buildForm();
        $previousData = Session::getInstance()->read("setup_data", []);
        Session::getInstance()->set("setup", Session::getInstance()->read("setup") + 1);
        Session::getInstance()->set("setup_data", array_merge($previousData, $form->getFields()));
        return $this->redirect("/setup");
    }

    public function backward(): Response
    {
        Session::getInstance()->set("setup", Session::getInstance()->read("setup") - 1);
        return $this->redirect("/setup");
    }
    ...
    private function setupConfigIniFile()
    {
        ...
        $configurations = [
            ...
            'session' => [
                ...
                'save_path' => $data['session_save_path'],
                ';name' => 'phpsessid',
                ';refresh_mode' => 'none',
                ';refresh_rate' => 0,
                ';lifetime' => 0,
        ...
        if (!empty($data['database_host'])) {
            $configurations['database']['hostname'] = $data['database_host'];
        }
...
//        }
    ...
    private function setupOthers()
    {
        ...
        if ($data['others_docker'] != '1') {
            (new File(ROOT_DIR . '/.docker-compose.yml'))->remove();
            (new Directory(ROOT_DIR . '/docker'))->remove();
        }
...
//    }
    ...
    private function formatDateTimeExample(string $timezone)
    {
        ...
        $formatter = new IntlDateFormatter(Locale::getDefault(), IntlDateFormatter::LONG, IntlDateFormatter::SHORT, null, null, Configuration::getLocaleConfiguration('format_datetime', Configuration::getLocaleConfiguration('format_datetime')));
